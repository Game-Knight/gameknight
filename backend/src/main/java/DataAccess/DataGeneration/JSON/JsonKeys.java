package DataAccess.DataGeneration.JSON;

public class JsonKeys {

    // Board Game Keys
    public static final String THUMBNAIL = "thumbnail";
    public static final String IMAGE = "image";
    public static final String NAME = "name";
    public static final String DESCRIPTION = "description";
    public static final String YEAR_PUBLISHED = "yearpublished";
    public static final String MIN_PLAYERS = "minplayers";
    public static final String MAX_PLAYERS = "maxplayers";
    public static final String PLAYING_TIME = "playingtime";
    public static final String MIN_PLAY_TIME = "minplaytime";
    public static final String MAX_PLAY_TIME = "maxplaytime";
    public static final String MIN_AGE = "minage";
    public static final String BGG_ID = "_id";
    public static final String VALUE = "_value";

    // UPC Keys
    public static final String UPC = "upc";
    public static final String UPC_BGG_ID = "bgg_id";

    // User Keys
    public static final String FIRST_NAME = "firstName";
    public static final String LAST_NAME = "lastName";
    public static final String PHONE_NUMBER = "phoneNumber";
    public static final String PASSWORD = "password";

    //Supplemental Data Keys
    public static final String RULES_URL = "rules_url";
}
